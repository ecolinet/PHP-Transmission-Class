...
class RPC
{
     ...
     */
    const HTTP_UA = 'TransmissionRPC for PHP/0.4.x-dev';
    ...
    private $default_context_opts = array(
        'http' => array(
            ...
            'timeout' => '5',
            // Leave the error parsing/handling to the code
            'ignore_errors' => true
        )
    );
     ...
     *
     * @param
     *            int|array ids A list of transmission torrent ids
     */
     ...
     * empty all torrents will be returned.
     * The fields array can be used to return certain
     ...
     *
     * @param
     *            array fields An array of return fields
     * @param
     *            int|array ids A list of transmission torrent ids
     */
     ...
     *
     * @param
     *            array arguments An associative array of arguments to set
     * @param
     *            int|array ids A list of transmission torrent ids
     */
     ...
     *
     * @param
     *            torrent_location The URL or path to the torrent file
     * @param
     *            save_path Folder to save torrent in
     * @param
     *            extra options Optional extra torrent options
     */
    public function add_file($torrent_location, $save_path = null, $extra_options = array())
    {
        ...
        if (\preg_match('|^https?://|', $torrent_location)) {
            $extra_options['filename'] = $torrent_location;
        } else {
            $file_content = file_get_contents($torrent_location);
            if (! $file_content) {
                throw new \RuntimeException("Unable to read file");
            }
            $extra_options['metainfo'] = base64_encode($file_content);
        }
     ...
     *
     * @param
     *            torrent_metainfo The raw, unencoded contents (metainfo) of a torrent
     * @param
     *            save_path Folder to save torrent in
     * @param
     *            extra options Optional extra torrent options
     ...
     *
     * @param
     *            bool delete_local_data Also remove local data?
     * @param
     *            int|array ids A list of transmission torrent ids
     */
     ...
     *            int|array ids A list of transmission torrent ids
     * @param
     *            string target_location The new storage location
     * @param
     *            string move_existing_data Move existing data or scan new location for available data
     */
     ...
     *
     * @return s array of session information
     */
     ...
     *
     * @param
     *            array of session variables to set
     */
     ...
     *
     * @param
     *            int The integer "torrent status"
     * @return s string The translated meaning
     */
    ...
    /**
     * Clean up the request array.
     * Removes any empty fields from the request
     *
     * @param
     *            array array The request associative array to clean
     * @return s array The cleaned array
     */
     ...
     * Replaces all minus(-) characters in the object properties with underscores
     * and converts any object with any all-digit property names to an array.
     *
     * @param
     *            object The request result to clean
     * @return s array The cleaned object
     */
     ...
     * @param
     *            array arguments The request arguments
     * @return s array The request result
     */
 ...
 */
    /*
     * public function GetSessionID() { if(!$this->url) { throw new RPCException("Class must be initialized before GetSessionID() can be called.", RPCException::E_INVALIDARG); } // Setup the context $contextopts = $this->default_context_opts; // Start with the defaults // Make sure it's blank/empty (reset) $this->session_id = null; // Setup authentication (if provided) if($this->username && $this->password) { $contextopts['http']['header'] = sprintf("Authorization: Basic %s\r\n", base64_encode($this->username.':'.$this->password)); } if($this->debug) { echo "TRANSMISSIONRPC_DEBUG:: GetSessionID():: Stream context created with options:". PHP_EOL . print_r($contextopts, true); } $context = stream_context_create($contextopts); // Create the context for this request if(! $fp = @fopen($this->url, 'r', false, $context)) { // Open a filepointer to the data, and use fgets to get the result throw new RPCException('Unable to connect to '.$this->url, RPCException::E_CONNECTION); } // Check the response (headers etc) $stream_meta = stream_get_meta_data($fp); fclose($fp); if($this->debug) { echo "TRANSMISSIONRPC_DEBUG:: GetSessionID():: Stream meta info: ". PHP_EOL . print_r($stream_meta, true); } if($stream_meta['timed_out']) { throw new RPCException("Timed out connecting to {$this->url}", RPCException::E_CONNECTION); } if(substr($stream_meta['wrapper_data'][0], 9, 3) == "401") { throw new RPCException("Invalid username/password.", RPCException::E_AUTHENTICATION); } elseif(substr($stream_meta['wrapper_data'][0], 9, 3) == "409") { // This is what we're hoping to find // Loop through the returned headers and extract the X-Transmission-Session-Id foreach($stream_meta['wrapper_data'] as $header) { if(strpos($header, 'X-Transmission-Session-Id: ') === 0) { if($this->debug) echo "TRANSMISSIONRPC_DEBUG:: GetSessionID():: Session-Id header: ". PHP_EOL . print_r($header, true); $this->session_id = trim(substr($header, 27)); break; } } if(! $this->session_id) { // Didn't find a session_id throw new RPCException("Unable to retrieve X-Transmission-Session-Id", RPCException::E_SESSIONID); } } else { throw new RPCException("Unexpected response from Transmission RPC: ".$stream_meta['wrapper_data'][0]); } return $this->session_id; }
     */
}
